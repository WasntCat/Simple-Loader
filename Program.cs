...
namespace Simple_Loader
{
    internal class Program
    {
        ...
        name: Encoding.Unicode.GetString(Convert.FromBase64String("SW0gQSBiZXR0ZXIgUGFzdGVyIENyeSBBYm91dCBpdCA8Mw")),
        ownerid: Encoding.ASCII.GetString(Convert.FromBase64String("Q2F0cyBBcmUgdGhlIGJlc3QgPDM")),
        secret: Encoding.UTF32.GetString(Convert.FromBase64String("AAAAZQAAAG0AAAAgAAAAZQAAAHQAAABhAAAAaAAAACAAAAB1AAAAbwAAAHkAAAAgAAAAdwAAAG8AAABuAAAAawAAACAAAABJAAAAIAAAADwAAAAz")),
        version: Encoding.UTF7.GetString(Convert.FromBase64String("SSBFbmpveSBtYWtpbmcgQmV0dGVyIFBhc3RlIHRoZW4geW91ICtBRHctMw"))
        );
        static void Main(string[] args)
        {
            ...
            if (File.Exists(@"C:\SimpleLoader.Darling"))
            {
        ...
        public static void BasicAutoInjMapping()
        {
            ...
            webClient.DownloadFile("", @"C:\Windows\IME\" + Driver);
            webClient.DownloadFile("", @"C:\Windows\IME\" + Mapper);
            webClient.DownloadFile("", @"C:\Windows\IME\" + Injector);

            utils.Start(@"C:\Windows\IME\" + Mapper, @"C:\Windows\IME\" + Driver); // Maping the driver

            //Logger.Text += "Starting Loader" + Environment.NewLine;
            //Logger.Text += "Changing to Loader Console" + Environment.NewLine;

            utils.Start($@"C:\Windows\IME\" + Injector); // Starting Your Injector Or Emulator
        }
